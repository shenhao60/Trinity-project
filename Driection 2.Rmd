...
mask_score <- mutate(mask_score,
       x =c(1:5) )
ggplot(data = mask_score, aes(x = x, y = overall_sentiment)) +
  geom_line()+
...
tweetsGeo_En$state <- tweetsGeo_En%>%
  mutate(state=revgeo(longitude=long, latitude=lat, output='frame',provider = 'bing', API='AhuhJ0p6HX3rF3tZ-9Tn9qJ2OSVF7HGz433SLbP1aJqQcNV4AIHqvB6c5orl6t6V'))
